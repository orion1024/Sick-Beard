...
class T411Provider(generic.TorrentProvider):
    ...
    def getSearchParams(self, searchString, audio_lang, subcat, french=None):
        if audio_lang == "en" and french==None:
            ...
            return urllib.urlencode( {'search': searchString, 'cat' : 210, 'submit' : 'Recherche', 'subcat': subcat } ) + "&term%5B51%5D%5B%5D=1210&term%5B51%5D%5B%5D=1212&term%5B51%5D%5B%5D=1215"
        else:
    ...
    def _doLogin(self, login, password):
        ...
        #self.opener.open(self.url + '/users/auth/', data)

        req = urllib2.Request(self.url + '/users/auth/', data, 
                                 {'Connection' : 'keep-alive', 'DNT' : '1', 'Referer' : self.url+'/' ,'X-Requested-With' : 'XMLHttpRequest','User-Agent' : 'Mozilla/5.0 (Windows NT 6.1; WOW64; rv:50.0) Gecko/20100101 Firefox/50.0'})
    ...
    def _doSearch(self, searchString, show=None, season=None, french=None):
        ...
        req.add_unredirected_header('User-Agent', 'Mozilla/5.0 (Windows NT 6.1; WOW64; rv:50.0) Gecko/20100101 Firefox/50.0')
